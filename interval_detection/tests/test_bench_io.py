...
def test_save_and_load_intervals_round_trip(tmp_path):
...
def test_unlabelled_vs_empty_are_distinct(tmp_path):
...
def test_default_and_invalid_type_coerced_to_other(tmp_path):
...
def test_save_drops_zero_or_negative_length(tmp_path):
...
def test_save_meta_preserves_intervals(tmp_path):
...
def test_save_meta_keeps_unlabelled_state(tmp_path):
...
def test_save_intervals_preserves_meta(tmp_path):
...
def test_load_activity_csv(tmp_path):
...
def test_list_activity_ids(tmp_path):
