...
def bench(tmp_path: Path) -> tuple[Path, Path]:
    ...
    labelio.save_intervals(
        "A", [(600, 1000, "vo2max"), (100, 140, "anaerobic")], labels_dir=labs
    )
    return acts, labs
...
def test_result_for_activity(bench: tuple[Path, Path]) -> None:
    acts, labs = bench
    r = view_results.result_for_activity(
        "A", ftp=250, activities_dir=acts, labels_dir=labs
    )
