...
class TestReloadPlans:
    def test_loads_original_plan_when_present(self, tmp_path: Path) -> None:
        ...
        with (
            patch("src.ai.chat_session.PLAN_ORIGINAL_PATH", plan_file),
    ...
    def test_loads_adapted_plan_when_present(self, tmp_path: Path) -> None:
        ...
        with (
            patch("src.ai.chat_session.PLAN_ORIGINAL_PATH", tmp_path / "missing.md"),
    ...
    def test_reload_updates_plans(self, tmp_path: Path) -> None:
        ...
        with (
            patch("src.ai.chat_session.PLAN_ORIGINAL_PATH", original),
            patch("src.ai.chat_session.PLAN_ADAPTED_PATH", tmp_path / "missing.md"),
        ):
            session = ChatSession()
            assert session.original_plan == ""
...
class TestBuildSystem:
    def _make_session(
        self, tmp_path: Path, original: str = "", adapted: str = ""
    ) -> ChatSession:
        ...
        with (
            patch("src.ai.chat_session.PLAN_ORIGINAL_PATH", orig),
            patch("src.ai.chat_session.PLAN_ADAPTED_PATH", adpt),
            patch(
                "src.ai.chat_session.SESSIONS_ORIGINAL_PATH", tmp_path / "sessions.csv"
            ),
            patch(
                "src.ai.chat_session.SESSIONS_ADAPTED_PATH",
                tmp_path / "sessions_adapted.csv",
            ),
        ):
            return ChatSession()
    ...
    def test_includes_both_plans_when_both_set(self, tmp_path: Path) -> None:
        session = self._make_session(
            tmp_path, original="# Original", adapted="# Adapted"
        )
        system = self._text(session)
        assert "Original Training Plan" in system
        assert "Adapted Training Plan" in system
    ...
    def test_original_plan_precedes_adapted_plan(self, tmp_path: Path) -> None:
        ...
        system = self._text(session)
        assert system.index("Original Training Plan") < system.index(
    ...
    def test_includes_session_table_when_sessions_present(self, tmp_path: Path) -> None:
        ...
        session = self._make_session(tmp_path)
        with (
            patch("src.ai.chat_session.SESSIONS_ORIGINAL_PATH", csv_file),
            patch(
                "src.ai.chat_session.SESSIONS_ADAPTED_PATH",
...
class TestBuildSessionTable:
    ...
    def _patches(
        ...
        adapted: Optional[Path] = None,
        log: Optional[Path] = None,
    ) -> ExitStack:
        stack = ExitStack()
        stack.enter_context(
            patch(
                "src.ai.chat_session.SESSIONS_ORIGINAL_PATH",
        ...
        stack.enter_context(
            patch(
                "src.ai.chat_session.SESSIONS_LOG_PATH",
    ...
    def test_returns_empty_when_all_sessions_outside_window(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_shows_completion_date_from_log(self, tmp_path: Path) -> None:
        ...
        self._write_csv(csv_file, [_csv_row(recent)])
        log_file.write_text(
            json.dumps({recent: {"completed_date": completed, "comment": ""}})
        )
    ...
    def test_shows_comment_from_log(self, tmp_path: Path) -> None:
        ...
        self._write_csv(csv_file, [_csv_row(recent)])
        log_file.write_text(
            json.dumps({recent: {"completed_date": recent, "comment": "felt strong"}})
        )
        with self._patches(tmp_path, original=csv_file, log=log_file):
            result = _build_session_table()
    ...
    def test_prefers_adapted_csv_over_original(self, tmp_path: Path) -> None:
        ...
        self._write_csv(
            adapted_csv, [_csv_row(recent, session_type="Threshold Intervals")]
        )
