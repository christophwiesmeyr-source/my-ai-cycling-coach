...
class TestClearDerivedPlanData:
    def test_deletes_adapted_plan_sessions_and_log(self, tmp_path: Path) -> None:
...
class TestBuildPlanHeader:
    ...
    def test_is_valid_markdown_table(self) -> None:
        goals = {"main_goal": "Race"}
        header = _build_plan_header(goals)
        assert "| Parameter | Value |" in header
        assert "|-----------|-------|" in header

    def test_ends_with_horizontal_rule(self) -> None:
        goals = {"main_goal": "Race"}
...
class TestBuildPlanPrompt:
    ...
    def test_contains_event_and_date(self) -> None:
        prompt = _build_plan_prompt(self.BASE_GOALS)
        assert "Gran Fondo" in prompt
        assert "2025-08-31" in prompt

    def test_contains_week_and_day_count(self) -> None:
        prompt = _build_plan_prompt(self.BASE_GOALS)
        assert "20 weeks" in prompt
        assert "140 days" in prompt

    def test_excludes_meta_keys_from_athlete_profile(self) -> None:
        prompt = _build_plan_prompt(self.BASE_GOALS)
...
class TestBuildSessionsPrompt:
    ...
    def test_contains_planning_context(self) -> None:
        prompt = _build_sessions_prompt(self.PLAN, self.GOALS)
...
class TestExtractCsv:
    ...
    def test_strips_preamble_before_header(self) -> None:
        raw = "Here is your CSV:\ndate,week,phase\n2025-04-01,1,Base"
        result = _extract_csv(raw)
        assert result.startswith("date,week,phase")
        assert "Here is your CSV" not in result

    def test_strips_markdown_fence_preamble(self) -> None:
        raw = "```csv\ndate,week,phase\n2025-04-01,1,Base\n```"
        result = _extract_csv(raw)
        assert result.startswith("date,week,phase")

    def test_preserves_all_data_rows(self) -> None:
        raw = "Preamble\ndate,week,phase\nrow1\nrow2\nrow3"
        result = _extract_csv(raw)
    ...
    def test_returns_raw_when_no_header_found(self) -> None:
